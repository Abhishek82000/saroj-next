...
/** Same bento shape as the static layout above, applied to live category tiles. */
    ...
    <section className="st-sec" style={{paddingBlock:"clamp(28px,5vw,54px)"}} id="shelf">
      <div className="st-wrap">
        ...
        <Reveal as="p" delay={2} className="st-lede">
          {live.length > 0
            ? `${categories.length} collections make up the counter. These ${live.length} open the shelf — the rest are a search away.`
            ...
            ? live.map((c, i) => (
              <RevealLink key={c.id} href={categoryHref(c.slug)}
                ...
                <div className="st-tile__body">
                  <span className="st-tile__name">{c.name}</span>
                </div>
              </RevealLink>
            ))
